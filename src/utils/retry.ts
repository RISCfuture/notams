...
const RETRIABLE_PG_CODES = new Set([
...
export const matchesConnectionError = (error: unknown): boolean => {
  if (typeof error !== 'object' || error === null) {
    return false
  }
  const err = error as { message?: string }
  return CONNECTION_ERROR_MESSAGES.some((msg) => err.message?.includes(msg) ?? false)
}
...
export const isRetriableDatabaseError = (error: unknown): boolean => {
  ...
  return (
    (err.code !== undefined && RETRIABLE_PG_CODES.has(err.code)) ||
    matchesConnectionError(error) ||
    (err.message?.includes('Client has encountered a connection error') ?? false)
  )
}
