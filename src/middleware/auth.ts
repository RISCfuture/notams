...
export const authenticateToken = async (
  ...
  try {
    ...
    const tokenRow = result.rows[0] as { id: number; name: string } | undefined
    if (!tokenRow) {
    ...
    void withRetry(() =>
      pool.query('UPDATE api_tokens SET last_used_at = NOW() WHERE id = $1', [tokenRow.id]),
    ).catch((err: unknown) => {
      logger.warn({ err }, 'Failed to update token last_used_at')
    })
    ...
    logger.debug({ tokenId: tokenRow.id, tokenName: tokenRow.name }, 'Token authenticated')
    next()
  } catch (error) {
